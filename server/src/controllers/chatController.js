...
12. GENERIC PYQ POLICY: If the user asks for PYQs, past year questions, or practice questions, but DOES NOT specify which exam type (CA, Mid Term, ETE, or ETP), you MUST ask them: "Are you looking for PYQs for a CA, Mid Term, ETE, or ETP?" Do not generate a massive list of questions until they specify the exam type.`;
        ...
        let dynamicSubjectRule = "";
        if (currentFilters.subject) {
            if (/^(MTH|PHY|CHE)/i.test(currentFilters.subject)) {
                dynamicSubjectRule = "SUBJECT RULE (MANDATORY): This is a Maths/Physics/Science subject. Subjective questions MUST be numerical problem-solving questions with actual values. Do NOT ask theoretical definitions.";
            } else if (/^(INT|CSE|MVC|ECE|CAP|PEL)/i.test(currentFilters.subject)) {
                dynamicSubjectRule = "SUBJECT RULE (MANDATORY): This is a Coding/Programming subject. Subjective questions MUST be CODE IMPLEMENTATION — ask the student to WRITE working code. Do NOT ask theoretical definitions.";
            } else {
                dynamicSubjectRule = "SUBJECT RULE: Provide a mix of short, medium, and long-answer questions.";
            }
        }
        ...
        if (isCasualChat) {
            ...
            const userSpecifiedUnits = /\b(unit|units)\s*\d\b/i.test(content);
            const courseKnown = !!currentFilters.subject;
            ...
            } else if (userSpecifiedType && userSpecifiedUnits && courseKnown) {
                userQueryFinal = content + `\n\n[REMINDER: CLASS ASSESSMENT (CA) request detected. All requirements (course, units, type) are met. Generate questions NOW. PYQ RULE: Extract ALL actual PYQ questions from the provided context FIRST for the specified units — do NOT skip any available PYQs. Only after exhausting real PYQs, invent new questions matching their pattern. SCOPE RULE: Every question MUST be strictly based on topics listed in the syllabus for the specified units. Do NOT ask about topics outside the syllabus. STYLE RULE (MANDATORY): Study the exact nature of the provided PYQ questions. If they are numerical, then EVERY question you generate MUST ALSO be numerical — give actual numbers, formulas, and values to compute. If code-based, give code to write. NEVER fall back to generic 'What is X?' theoretical questions unless PYQs are purely theoretical. ${dynamicSubjectRule} (For MCQs: Generate EXACTLY 30 MCQs). DO NOT stop early.]`
            } else {
                userQueryFinal = content + `\n\n[REMINDER: CLASS ASSESSMENT (CA) request detected. You need 3 pieces of information. Course known: ${courseKnown ? 'YES' : 'NO'}. Units specified: ${userSpecifiedUnits ? 'YES' : 'NO'}. Type specified: ${userSpecifiedType ? 'YES' : 'NO'}. Ask the user ONLY for the missing pieces of information. CRITICAL: You are FORBIDDEN from generating any questions in this response. Your ONLY job right now is to ask the clarifying questions and then STOP GENERATING.]`;
            }
